...
use otlp::{
    error::ErrorCategory as OtlpCategory, error::ErrorSeverity as OtlpSeverity, error::OtlpError,
};
use reliability::{
    chaos_engineering::{ChaosEngineeringConfig, ChaosEngineeringManager},
    config::ConfigManager,
    //log_error,
    error_handling::{ErrorHandler, RecoveryStrategy},
    fault_tolerance::{
        CircuitBreakerConfig, FallbackConfig, Retrier, RetryConfig, RetryPolicy, TimeoutConfig,
    },
...
async fn error_handling_example() -> Result<(), OtlpError> {
    // 创建统一错误
    let _ctx = otlp::error::ErrorContext::new(OtlpCategory::Processing, OtlpSeverity::Medium);
    ...
    let result = if rand::random::<bool>() {
        ...
        Err(otlp_convert::to_otlp_error(&UnifiedError::new(
            "操作失败",
            ErrorSeverity::Low,
            "example",
            ErrorContext::new(
...
/// 容错机制示例
#[allow(unused_variables)]
async fn fault_tolerance_example() -> Result<(), OtlpError> {
    // 创建断路器
    ...
    let result = circuit_breaker
        .execute(|| async {
            ...
            if rand::random::<bool>() {
                Ok::<String, UnifiedError>("操作成功".to_string())
            } else {
...
/// 运行时监控示例
async fn runtime_monitoring_example() -> Result<(), OtlpError> {
    // 创建健康检查器
...
/// 混沌工程示例
async fn chaos_engineering_example() -> Result<(), OtlpError> {
    // 创建混沌工程管理器
    ...
    println!(
        "  总体评分: {:.2}",
        test_result.overall_assessment.overall_score
    );
    println!(
        "  弹性评分: {:.2}",
        test_result.overall_assessment.resilience_score
    );
    println!(
        "  恢复评分: {:.2}",
        test_result.overall_assessment.recovery_score
    );
    println!(
        "  建议: {:?}",
        test_result.overall_assessment.recommendations
    );
...
/// 配置管理示例
async fn config_management_example() -> Result<(), OtlpError> {
    // 创建配置管理器
    ...
    println!(
        "当前配置: 应用名称: {}, 环境: {}",
        current_config.global.app_name, current_config.global.environment
    );
...
/// 指标收集示例
async fn metrics_collection_example() -> Result<(), OtlpError> {
    // 创建指标收集器
...
/// 工具函数示例
async fn utility_functions_example() -> Result<(), OtlpError> {
    // 持续时间扩展
