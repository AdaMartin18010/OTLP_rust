...
use reliability::fault_tolerance::{CircuitBreaker, CircuitBreakerConfig, RetryPolicy, RetryConfig};
use reliability::runtime_monitoring::{HealthChecker, HealthCheckConfig};
use reliability::config::{ConfigManager, ReliabilityConfig};
use reliability::error_handling::UnifiedError;

#[tokio::test]
async fn test_full_reliability_workflow() {
    // 初始化框架
    assert!(reliability::init().await.is_ok());
    ...
    // 关闭框架
    assert!(reliability::shutdown().await.is_ok());
}
