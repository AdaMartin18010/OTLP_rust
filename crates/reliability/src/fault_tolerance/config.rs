...
use super::{
    BulkheadConfig, CircuitBreakerConfig, FallbackCondition, FallbackConfig, FallbackStrategy,
    RetryConfig, RetryStrategy, TimeoutConfig,
};
...
impl FaultToleranceConfigValidator {
    ...
    fn validate_timeout(config: &TimeoutConfig) -> Result<(), UnifiedError> {
        if config.duration == Duration::ZERO {
            return Err(Self::create_validation_error("超时时间不能为0", "timeout"));
        }
        ...
        if config.name.is_empty() {
            return Err(Self::create_validation_error("超时名称不能为空", "timeout"));
        }
    ...
    fn create_validation_error(message: &str, category: &str) -> UnifiedError {
        ...
        UnifiedError::new(message, ErrorSeverity::High, category, context)
...
impl FaultToleranceConfigManager {
    ...
    pub fn load_from_file(&mut self, path: &str) -> Result<(), UnifiedError> {
        let content =
            std::fs::read_to_string(path).map_err(|e| Self::create_file_error(&e, path))?;

        let config: FaultToleranceConfig =
            toml::from_str(&content).map_err(|e| Self::create_parse_error(&e, path))?;
    ...
    pub fn save_to_file(&self, path: &str) -> Result<(), UnifiedError> {
        let content =
            toml::to_string_pretty(&self.config).map_err(|e| Self::create_serialize_error(&e))?;

        std::fs::write(path, content).map_err(|e| Self::create_file_error(&e, path))?;
