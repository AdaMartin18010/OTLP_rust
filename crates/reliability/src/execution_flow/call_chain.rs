...
impl CallChainTracker {
    ...
    pub fn end_call(&self, call_id: &CallId, status: &str, error: Option<String>) {
        ...
        for mut entry in self.active_chains.iter_mut() {
            if let Some(call_entry) = entry.value_mut().iter_mut().find(|e| &e.call_id == call_id) {
                call_entry.duration_us =
                    Instant::now().elapsed().as_micros() as u64 - call_entry.start_time;
    ...
    pub fn get_stats(&self) -> CallChainStats {
        ...
        let p95_duration_us = durations
            .get(durations.len() * 95 / 100)
            .copied()
            .unwrap_or(0);
        let p99_duration_us = durations
    ...
    pub fn cleanup_old_chains(&self, older_than: Duration) {
        let now = Instant::now();
        self.completed_chains
            .retain(|_, chain| chain.total_duration < now.elapsed() - older_than);
    }
}
...
mod tests {
    ...
    fn test_call_chain_tracker() {
        ...
        let call_id =
            tracker.start_call("test_method".to_string(), "test_service".to_string(), None);
    ...
    fn test_nested_calls() {
        ...
        let parent_id = tracker.start_call("parent".to_string(), "service".to_string(), None);
