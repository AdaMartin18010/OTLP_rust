...
impl EmbeddedEnvironmentAdapter {
    ...
    /// 创建带自定义配置的嵌入式环境适配器
    pub fn with_config(total_memory: u64, total_cpu_cores: u32, total_disk_space: u64) -> Self {
        Self {
            system_name: "Embedded Bare Metal".to_string(),
            system_version: "1.0.0".to_string(),
            architecture: std::env::consts::ARCH.to_string(),
    ...
    fn get_hardware_info(&self) -> HashMap<String, String> {
        ...
        info.insert(
            ...
            self.interrupt_count.to_string(),
        );
...
impl RuntimeEnvironmentAdapter for EmbeddedEnvironmentAdapter {
    ...
    async fn get_resource_usage(&self) -> Result<ResourceUsage, UnifiedError> {
        ...
        let memory_usage_percent =
            (adapter.current_memory_usage as f64 / adapter.total_memory as f64) * 100.0;
    ...
    async fn check_health(&self) -> Result<HealthStatus, UnifiedError> {
        ...
        // 检查内存使用率
        let memory_usage_percent =
            (self.current_memory_usage as f64 / self.total_memory as f64) * 100.0;
        ...
        environment_specific.insert(
            "cpu_usage".to_string(),
            format!("{:.1}%", self.current_cpu_usage),
        );
...
mod tests {
    ...
    async fn test_embedded_environment_adapter() {
        ...
        assert!(matches!(
            ...
            HealthLevel::Healthy | HealthLevel::Warning | HealthLevel::Critical
        ));
