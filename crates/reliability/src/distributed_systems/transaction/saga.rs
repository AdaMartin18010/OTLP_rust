...
use super::{
    DistributedTransaction, StepExecution, StepResult, TransactionContext, TransactionId,
    TransactionMetrics, TransactionState, TransactionStep,
};
...
impl SagaCoordinator {
    ...
    /// 执行 Saga (编排式)
    async fn execute_orchestration(&mut self, tx_id: &TransactionId) -> Result<(), UnifiedError> {
        let mut context = {
        ...
        for i in (0..executed_count).rev() {
            if let Some(step) = self.steps.get_mut(i) {
                if let Err(e) = step.compensate(context).await {
                    tracing::error!("补偿步骤 {} 失败: {:?}", step.name(), e);
                    // 继续补偿其他步骤
                }
