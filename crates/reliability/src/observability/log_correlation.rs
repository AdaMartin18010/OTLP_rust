...
//! ## Rust 1.92 特性应用
//!
//! - **异步闭包**: 使用 `async || {}` 语法简化异步日志关联操作
//! - **元组收集**: 使用 `collect()` 直接收集日志关联数据到元组
//! - **改进的日志关联**: 利用 Rust 1.92 的日志关联优化提升性能
use crate::prelude::*;
...
impl LogCorrelator {
    ...
    pub async fn log(
        ...
        self.log_with_fields(context, level, message, HashMap::new())
            .await
    }
...
mod tests {
    ...
    async fn test_log_correlation() {
        ...
        let context = CorrelationContext::new("test-service").with_user_id("user123");

        correlator
            .log(context.clone(), LogLevel::Info, "Test log 1")
            .await
            .unwrap();
        correlator
            .log(context.clone(), LogLevel::Error, "Test log 2")
            .await
            .unwrap();
    ...
    async fn test_log_query() {
        ...
        correlator
            .log(context.clone(), LogLevel::Debug, "Debug log")
            .await
            .unwrap();
        correlator
            .log(context.clone(), LogLevel::Error, "Error log")
            .await
            .unwrap();
