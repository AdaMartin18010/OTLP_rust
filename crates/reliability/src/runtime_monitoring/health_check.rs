...
impl HealthChecker {
    ...
    /// 执行健康检查
    pub async fn check_health(&self) -> otlp::error::Result<HealthCheckResult> {
    ...
    async fn execute_check(&self, item: &HealthCheckItem) -> HealthCheckItemResult {
        ...
        match item.check_type {
            ...
            HealthCheckType::Database => {
                ...
                details.insert(
                    "status".to_string(),
                    "database_check_not_implemented".to_string(),
                );
            ...
            HealthCheckType::Custom {
                ...
                details.insert(
                    "status".to_string(),
                    "custom_check_not_implemented".to_string(),
                );
    ...
    async fn check_disk_health(&self) -> HealthCheckItemResult {
        ...
        details.insert(
            "overall_usage_percent".to_string(),
            format!("{:.2}", overall_usage_percent),
        );
    ...
    pub fn register_handler(
        &self,
        name: String,
        handler: Box<dyn HealthCheckHandler + Send + Sync>,
    ) {
...
pub trait HealthCheckHandler: Send + Sync {
    ...
    fn check(
        ...
        Box<dyn std::future::Future<Output = Result<HealthCheckItemResult, UnifiedError>> + Send>,
    >;
}
...
impl GlobalHealthChecker {
    ...
    /// 初始化全局健康检查器
    pub async fn init_global() -> otlp::error::Result<()> {
        let checker = GlobalHealthChecker::new();
        checker.start().await
    }

    /// 关闭全局健康检查器
    pub async fn shutdown_global() -> otlp::error::Result<()> {
        Ok(())
    }
}
