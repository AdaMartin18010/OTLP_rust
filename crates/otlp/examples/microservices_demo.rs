...
async fn demo_service_discovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 模拟调用微服务
    info!("📞 调用微服务API");
    let res = microservice_client
        .call_service("user-service", |endpoint| {
            let url = format!(
                "http://{}:{}/api/users/123",
...
async fn demo_complete_microservice_architecture() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let order_service_endpoints = vec![ServiceEndpoint {
    ...
    user_service_lb
        .update_endpoints(user_service_endpoints.clone())
        .await;

    for i in 0..3 {
        if let Some(endpoint) = user_service_lb
            .select_endpoint(&user_service_endpoints)
            .await
        {
            info!(
                "调用用户服务 #{}: {}:{}",
            ...
            order_service_lb
                .update_endpoints(order_service_endpoints.clone())
                .await;

            if let Some(order_endpoint) = order_service_lb
                .select_endpoint(&order_service_endpoints)
                .await
            {
                info!(
                    "调用订单服务: {}:{}",
                    order_endpoint.address, order_endpoint.port
                );
