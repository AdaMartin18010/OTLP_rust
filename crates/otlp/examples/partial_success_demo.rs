...
use otlp::response::{
    ...
    ResponseAggregator, ResponseHandler, ResponseHandlerBuilder, ResponseMetadata,
    ResponseMetricsCollector, ResponseType, SignalType,
};
...
fn main() {
    ...
    println!(
        "   Success rate: {:.1}%",
        trace_handler.success_rate() * 100.0
    );
    println!(
        "   Is acceptable (< 10% rejection): {}",
        trace_handler.is_acceptable(0.1)
    );
    println!();
    ...
    println!(
        "   Acceptable at 20% threshold: {}",
        metrics_handler.rejected_count() as f64 / 100.0 <= 0.20
    );
    println!(
        "   Acceptable at 10% threshold: {}",
