...
- [OTLP Rust 迁移指南和版本升级说明](#otlp-rust-迁移指南和版本升级说明)
  - [📋 目录](#-目录)
  - [版本升级策略](#版本升级策略)
    - [版本号规范](#版本号规范)
    - [升级路径](#升级路径)
  ...
  - [回滚策略](#回滚策略)
    ...
    - [3. 数据库回滚](#3-数据库回滚)
  - [总结](#总结)
  - [Rust 1.90 对齐与迁移检查清单](#rust-190-对齐与迁移检查清单)
    - [1) 编译环境与工具链](#1-编译环境与工具链)
    - [2) 依赖与特性开关](#2-依赖与特性开关)
    - [3) 破坏性变更对齐（示例）](#3-破坏性变更对齐示例)
    - [4) 验证与示例运行](#4-验证与示例运行)
    - [5) 源码映射（Spec ↔ Code）](#5-源码映射spec--code)
    - [6) 回归检查清单（升级前/后均需）](#6-回归检查清单升级前后均需)
    - [7) 常用脚本与命令汇总](#7-常用脚本与命令汇总)
