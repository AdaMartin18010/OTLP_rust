...
impl OttlParser {
    ...
    fn parse_set(statement: &str) -> Result<OttlStatement> {
        ...
        let inner = statement.trim_start_matches("set(").trim_end_matches(")");
    ...
    fn parse_comparison(s: &str) -> Result<OttlCondition> {
        ...
        Err(crate::error::OtlpError::ValidationError(format!(
            "Invalid condition: {}",
            s
        )))
    }
...
impl OttlProcessor {
    ...
    pub fn parse_and_add_with_condition(
        &mut self,
        statement_str: &str,
        condition_str: &str,
    ) -> Result<()> {
    ...
    fn set_span_attribute(
        ...
        let OttlPath::Identifier(base_name) = base else {
            return;
        };
    ...
    fn execute_keep_keys(
        &self,
        _map: &OttlPath,
        _keys: &[String],
        _ctx: &mut OttlContext,
    ) -> Result<()> {
...
mod tests {
    ...
    fn test_processor_execute() {
        ...
        processor
            .parse_and_add("set(attributes[\"service\"], \"test\")")
            .unwrap();
    ...
    fn test_parse_condition_logical_and() {
        let condition =
            OttlParser::parse_condition("span.duration > 1000 and span.status == \"error\"")
                .unwrap();
    ...
    fn test_parse_condition_logical_or() {
        let condition =
            OttlParser::parse_condition("span.status == \"error\" or span.status == \"unset\"")
                .unwrap();
