...
impl PprofBuilder {
    ...
    pub fn create_sample_from_stack(&mut self, stack_frames: &[StackFrame], value: i64) -> Sample {
...
impl PprofEncoder {
    ...
    pub fn save_json(profile: &PprofProfile, path: &std::path::Path) -> Result<(), String> {
        ...
        std::fs::write(path, data).map_err(|e| format!("Failed to write profile to file: {}", e))
    }

    /// Load profile from file (JSON format)
    pub fn load_json(path: &std::path::Path) -> Result<PprofProfile, String> {
        let data =
            std::fs::read(path).map_err(|e| format!("Failed to read profile from file: {}", e))?;
