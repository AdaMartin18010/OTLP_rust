...
pub async fn complete_otlp_pipeline_demo() -> Result<PipelineResult, Box<dyn std::error::Error>> {
    ...
    println!(
        "    - EULER_GAMMA^2: {:.10}",
        AdaptiveSampler::EULER_GAMMA_SQUARED
    );
    println!(
        "    - 1 + EULER_GAMMA: {:.10}",
        AdaptiveSampler::ONE_PLUS_GAMMA
    );
    println!("    - Pre-computed rate (low): {:.3}", ConstMath::RATE_LOW);
    println!(
        "    - Pre-computed rate (high): {:.3}",
        ConstMath::RATE_HIGH
    );
    ...
    println!(
        "    - FP16 available: {}",
        Fp16MetricsProcessor::is_available()
    );
    ...
    println!(
        "    - 3-point moving average (first 5): {:?}",
        &ma3[..ma3.len().min(5)]
    );
...
mod tests {
    ...
    fn test_golden_ratio_backoff() {
        ...
        assert!(
            (160..=165).contains(&d1),
            "d1 ({}) should be approximately 161-162",
            d1
        );
        assert!(
            (260..=265).contains(&d2),
            "d2 ({}) should be approximately 261-262",
            d2
        );
    }
