...
fn test_gen_ai_semantic_conventions() {
    use otlp::semantic_conventions::gen_ai::{
        GenAiAttributes, GenAiFinishReason, GenAiOperation, GenAiSystem,
    };
...
"#;
    ...
    let config = OpenTelemetryConfig::from_yaml(yaml).expect("Failed to parse config");
...
fn test_ottl_processor_basic() {
    use otlp::ottl::processor::{OttlContext, OttlProcessor};

    let mut processor = OttlProcessor::new();
    processor
        .parse_and_add("set(attributes[\"key\"], \"value\")")
...
async fn test_profiling_lifecycle() {
    ...
    // Start profiling
    profiler.start().await.expect("Failed to start profiler");
    ...
    // Stop profiling
    profiler.stop().await.expect("Failed to stop profiler");
    assert!(!profiler.is_running());
}
...
async fn test_cpu_profiler_lifecycle() {
    ...
    // Stop profiler
    profiler.stop().await.expect("Failed to stop CPU profiler");
}
...
fn test_telemetry_data_creation() {
    use otlp::data::{TelemetryContent, TelemetryData, TelemetryDataType, TraceData};
...
async fn test_profile_exporter() {
    use otlp::profiling::exporter::{ProfileExporter, ProfileExporterConfig, generate_profile_id};
