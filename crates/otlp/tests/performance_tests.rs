...
async fn test_otlp_latency_performance() {
    ...
    let component = OtlpComponent::new("latency_test".to_string(), Duration::from_millis(1));
    ...
    assert!(
        ...
        "Average latency should be less than 10ms, got {:?}",
        avg_latency
    );
    assert!(
        max_latency < Duration::from_millis(50),
        "Max latency should be less than 50ms, got {:?}",
        max_latency
    );
    assert!(
        min_latency >= Duration::from_millis(1),
        "Min latency should be at least 1ms, got {:?}",
        min_latency
    );
...
async fn test_otlp_throughput_performance() {
    ...
    assert!(
        throughput > 100.0,
        "Throughput should be greater than 100 ops/sec, got {:.2}",
        throughput
    );
...
async fn test_otlp_memory_performance() {
    let component = OtlpComponent::new("memory_test".to_string(), Duration::from_millis(1));
...
async fn test_otlp_concurrent_performance() {
    ...
    assert!(
        throughput > 10.0,
        "Concurrent throughput should be greater than 10 ops/sec, got {:.2}",
        throughput
    );
...
async fn test_otlp_stress_performance() {
    ...
    assert!(
        operations > 100,
        "Should complete more than 100 operations in stress test, got {}",
        operations
    );
    assert!(
        throughput > 20.0,
        "Stress test throughput should be greater than 20 ops/sec, got {:.2}",
        throughput
    );
...
async fn test_otlp_error_performance() {
    let component = OtlpComponent::new("error_test".to_string(), Duration::from_millis(1));
    ...
    for _ in 0..50 {
        let _result = component
            .process(valid_data)
            .await
            .expect("Should process valid data");
    }
    ...
    assert!(
        ...
        "Error handling should not significantly impact performance, got {:?}",
        duration
    );
...
async fn test_otlp_resource_usage() {
    let component = OtlpComponent::new("resource_test".to_string(), Duration::from_millis(1));
    ...
    assert!(
        throughput > 50.0,
        "Resource usage should be efficient, throughput should be greater than 50 ops/sec, got {:.2}",
        throughput
    );
