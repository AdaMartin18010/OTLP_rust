...
use otlp::network::{
    AsyncIoConfig, AsyncIoManager, ConnectionPool, ConnectionPoolConfig, LoadBalancer,
    LoadBalancerConfig, LoadBalancingStrategy, NetworkConfig, NetworkManager,
};
...
fn bench_connection_pool_operations(c: &mut Criterion) {
    ...
    let pool = rt.block_on(async {
        ConnectionPool::new(config, io_config, addresses)
            .await
            .unwrap()
    });
...
fn bench_network_manager_operations(c: &mut Criterion) {
    ...
    let config = NetworkConfig::default();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let manager = rt.block_on(async { NetworkManager::new(config).await.unwrap() });
...
fn bench_concurrent_operations(c: &mut Criterion) {
    ...
    let config = NetworkConfig::default();
    let rt = tokio::runtime::Runtime::new().unwrap();
    let manager = Arc::new(rt.block_on(async { NetworkManager::new(config).await.unwrap() }));
            ...
            rt.block_on(async {
                let handles: Vec<_> = (0..10)
                    .map(|_| {
                        ...
                        tokio::spawn(async move { manager.get_stats().await })
                    })
                    .collect();
    ...
    group.bench_function("create_multiple_managers", |b| {
        b.iter(|| {
            ...
            let managers: Vec<_> = (0..100)
                .map(|_| rt.block_on(async { NetworkManager::new(config.clone()).await.unwrap() }))
                .collect();
            black_box(managers)
        })
    });
