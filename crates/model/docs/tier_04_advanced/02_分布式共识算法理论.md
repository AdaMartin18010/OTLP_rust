...
- [分布式共识算法理论](#分布式共识算法理论)
  ...
  - [🎯 概述](#-概述)
    - [共识问题形式化定义](#共识问题形式化定义)
  - [📝 CAP定理与一致性模型](#-cap定理与一致性模型)
    - [2.1 CAP定理](#21-cap定理)
    - [2.2 一致性模型层次](#22-一致性模型层次)
  - [💡 FLP不可能性](#-flp不可能性)
    - [3.1 FLP定理](#31-flp定理)
  - [🔧 Paxos理论深度分析](#-paxos理论深度分析)
...
impl PBFTNode {
    ...
    fn verify_signature(&self, message: &PBFTMessage) -> bool {
        ...
        // let verifying_key = VerifyingKey::from_bytes(&message.signer_public_key)?;
        // verifying_key.verify(message.payload.as_bytes(), &message.signature).is_ok()
        true
    }
}
```
