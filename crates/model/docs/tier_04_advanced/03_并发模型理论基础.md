...
- [并发模型理论基础](#并发模型理论基础)
  ...
  - [💡 Petri网理论](#-petri网理论)
    - [3.1 基本定义](#31-基本定义)
  - [🔧 Session Types深度解析](#-session-types深度解析)
...
impl<T, S, E> Chan<Recv<T, S>, E> {
    pub fn recv(self) -> (T, Chan<S, E>) {
        // 从对端接收
        // 实际实现示例:
        // let (value, next_chan) = self.channel.recv().await?;
        // (value, Chan { channel: next_chan, _session: PhantomData, _endpoint: PhantomData })
        // 注意：这是类型级会话类型的实现，实际运行时需要底层通道支持
...
impl<L, R, E> Chan<Offer<L, R>, E> {
        ...
        G: FnOnce(Chan<R, E>) -> T,
    {
        // 根据对端选择执行相应分支
        // 实际实现示例:
        // match self.channel.recv_choice().await? {
        //     Choice::Left(next_chan) => left(Chan { channel: next_chan, _session: PhantomData, _endpoint: PhantomData }),
        //     Choice::Right(next_chan) => right(Chan { channel: next_chan, _session: PhantomData, _endpoint: PhantomData }),
        // }
        // 注意：这是类型级会话类型的实现，实际运行时需要底层通道支持
        left(unsafe { std::mem::zeroed() })
    }
}
