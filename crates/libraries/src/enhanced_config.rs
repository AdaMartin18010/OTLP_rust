...
//! ## Rust 1.92 特性应用
//!
//! - **常量泛型**: 使用常量泛型优化配置结构大小和验证
//! - **改进的配置管理**: 利用 Rust 1.92 的配置管理优化提升性能
//! - **元组收集**: 使用 `collect()` 直接收集配置项到元组

use crate::config::{RetryPolicy, Timeouts};

/// Rust 1.92 特性：使用常量泛型优化配置结构
///
