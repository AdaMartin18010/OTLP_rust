    ...
    fn check_container_health(&self) -> HealthLevel {
        ...
        if let Some(memory_limit) = self.resource_limits.memory_limit {
            let memory_usage_percent =
                (self.current_usage.memory_usage as f64 / memory_limit as f64) * 100.0;
...
impl RuntimeEnvironmentAdapter for ContainerEnvironmentAdapter {
    ...
    async fn check_health(&self) -> Result<HealthStatus, UnifiedError> {
        ...
        environment_specific.insert(
            ...
            format!(
                "{:.1}%",
                (self.current_usage.memory_usage as f64
                    / self.resource_limits.memory_limit.unwrap_or(1) as f64)
                    * 100.0
            ),
        );
        environment_specific.insert(
            "cpu_usage".to_string(),
            format!("{:.1}%", self.current_usage.cpu_usage),
        );
