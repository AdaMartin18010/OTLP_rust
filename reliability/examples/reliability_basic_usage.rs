...
// 优先采用 OTLP 语义类型
use otlp::{error::OtlpError, error::ErrorSeverity as OtlpSeverity, error::ErrorCategory as OtlpCategory};
use reliability::{
    //log_error,
...
async fn fault_tolerance_example() -> Result<(), OtlpError> {
    ...
    let result = circuit_breaker.execute(|| async {
            ...
            if rand::random::<bool>() { Ok::<String, UnifiedError>("操作成功".to_string()) } else { Err(UnifiedError::new(
                "操作失败",
