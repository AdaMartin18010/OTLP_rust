...
use reliability::{
    ...
    runtime_monitoring::{HealthCheck, HealthCheckConfig, HealthStatus, ResourceMonitorConfig},
};
...
impl AppService {
    /// 创建新的应用服务
    async fn new() -> std::result::Result<Self, otlp::error::OtlpError> {
        // 创建健康检查器
    ...
    /// 启动应用服务
    async fn start(&mut self) -> std::result::Result<(), otlp::error::OtlpError> {
        println!("启动应用服务...");
    ...
    async fn process_request(
        &self,
        request: Request,
    ) -> std::result::Result<Response, otlp::error::OtlpError> {
        ...
        let result = self
            .circuit_breaker
            .execute(|| async {
                // 实际的业务逻辑处理（返回 UnifiedError 以符合执行器约束）
        ...
        result.map_err(|e| otlp_convert::to_otlp_error(&e))
    }
    ...
    async fn handle_business_logic(
        &self,
        request: &Request,
    ) -> std::result::Result<Response, UnifiedError> {
        // 模拟数据库操作
    ...
    async fn simulate_database_operation(
        &self,
        data: &str,
    ) -> std::result::Result<String, UnifiedError> {
        // 模拟数据库延迟
    ...
    /// 模拟缓存操作
    async fn simulate_cache_operation(
        &self,
        data: &str,
    ) -> std::result::Result<String, UnifiedError> {
        // 模拟缓存延迟
    ...
    /// 模拟外部服务调用
    async fn simulate_external_service_call(
        &self,
        data: &str,
    ) -> std::result::Result<String, UnifiedError> {
        // 模拟网络延迟
    ...
    /// 停止应用服务
    async fn stop(&mut self) -> std::result::Result<(), otlp::error::OtlpError> {
        println!("停止应用服务...");
        ...
        println!(
            "  总请求数: {}",
            final_metrics.performance_metrics.total_requests
        );
        println!(
            "  成功请求数: {}",
            final_metrics.performance_metrics.successful_requests
        );
        println!(
            "  失败请求数: {}",
            final_metrics.performance_metrics.failed_requests
        );
        println!(
            "  成功率: {:.2}%",
            (final_metrics.performance_metrics.successful_requests as f64
                / final_metrics.performance_metrics.total_requests as f64)
                * 100.0
        );
        println!(
            "  平均响应时间: {:?}",
            final_metrics.performance_metrics.average_response_time
        );
        println!("  总错误数: {}", final_metrics.error_metrics.total_errors);
        println!(
            "  错误率: {:.2}%",
            final_metrics.error_metrics.error_rate * 100.0
        );
