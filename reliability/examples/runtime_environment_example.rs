...
#[tokio::main]
async fn main() -> Result<(), otlp::error::OtlpError> {
    // 初始化日志
...
/// 演示操作系统环境的使用
async fn demonstrate_os_environment() -> Result<(), otlp::error::OtlpError> {
    println!("创建操作系统环境适配器...");
    
    let mut adapter = OSEnvironmentAdapter::new();
    adapter.initialize().await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    
    // 获取系统信息
    let system_info = adapter.get_system_info().await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    println!("系统信息:");
    ...
    // 获取资源使用情况
    let resource_usage = adapter.get_resource_usage().await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    println!("资源使用情况:");
    ...
    // 检查健康状态
    let health_status = adapter.check_health().await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    println!("健康状态:");
    ...
    // 执行恢复操作
    println!("执行内存清理...");
    adapter.perform_recovery(RecoveryType::MemoryCleanup).await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    
    adapter.cleanup().await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    println!("操作系统环境示例完成");
...
/// 演示嵌入式裸机环境的使用
async fn demonstrate_embedded_environment() -> Result<(), otlp::error::OtlpError> {
...
/// 演示Docker容器环境的使用
async fn demonstrate_container_environment() -> Result<(), otlp::error::OtlpError> {
...
/// 演示环境自适应使用
async fn demonstrate_adaptive_usage() -> Result<(), otlp::error::OtlpError> {
    println!("演示环境自适应使用...");
    ...
    // 初始化环境
    manager.initialize().await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    ...
    // 根据环境能力调整可靠性策略
    adjust_reliability_strategy(&capabilities).await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    
    // 清理环境
    manager.cleanup().await.map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    println!("环境自适应使用示例完成");
