...
use reliability::runtime_environments::{
    ContainerEnvironmentAdapter, EdgeComputingEnvironmentAdapter, EmbeddedEnvironmentAdapter,
    EnvironmentCapabilities, FaaSEnvironmentAdapter, OSEnvironmentAdapter,
    RealTimeOSEnvironmentAdapter, RuntimeEnvironment, RuntimeEnvironmentManager,
...
async fn demonstrate_os_environment() -> Result<(), otlp::error::OtlpError> {
    ...
    adapter
        .initialize()
        .await
        .map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;

    // 获取系统信息
    let system_info = adapter
        .get_system_info()
        .await
        .map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
    println!("系统信息:");
    ...
    println!(
        "  内存使用: {} bytes ({:.1}%)",
        resource_usage.memory_usage_bytes, resource_usage.memory_usage_percent
    );
    println!(
        "  磁盘使用: {} bytes ({:.1}%)",
        resource_usage.disk_usage_bytes, resource_usage.disk_usage_percent
    );
    ...
    println!("执行内存清理...");
    adapter
        .perform_recovery(RecoveryType::MemoryCleanup)
        .await
        .map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
...
async fn demonstrate_adaptive_usage() -> Result<(), otlp::error::OtlpError> {
    ...
    println!(
        "  支持内存管理: {}",
        capabilities.supports_memory_management
    );
    println!(
        "  支持进程管理: {}",
        capabilities.supports_process_management
    );
    ...
    // 根据环境能力调整可靠性策略
    adjust_reliability_strategy(&capabilities)
        .await
        .map_err(|e| reliability::otlp_integration::convert::to_otlp_error(&e))?;
...
/// 根据环境能力调整可靠性策略
async fn adjust_reliability_strategy(
