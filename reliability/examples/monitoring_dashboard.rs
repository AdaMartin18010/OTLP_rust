//! 监控仪表板示例
//!
//! 展示 reliability 库中的监控仪表板功能，包括实时数据收集、历史数据分析和统计摘要。
...
fn display_data_point(data_point: &MonitoringDataPoint, _iteration: u32) {
    println!(
        "   🕐 时间: {}",
        chrono::DateTime::<chrono::Utc>::from(data_point.timestamp).format("%H:%M:%S")
    );
    ...
    println!(
        "   {} 健康状态: {:?}",
        health_emoji, data_point.health_status
    );
    ...
    println!(
        "      CPU: {:.1}%",
        data_point.resource_usage.cpu_usage_percent
    );
    println!(
        "      内存: {:.1}%",
        data_point.resource_usage.memory_usage_percent
    );
    println!(
        "      磁盘: {:.1}%",
        data_point.resource_usage.disk_usage_percent
    );
    println!(
        "      网络: {:.1}%",
        data_point.resource_usage.network_io_usage
    );
    println!(
        "      连接: {}",
        data_point.resource_usage.active_connections
    );
    ...
    println!(
        "      平均响应时间: {:?}",
        data_point.performance_metrics.average_response_time
    );
    println!(
        "      P95响应时间: {:?}",
        data_point.performance_metrics.p95_response_time
    );
    println!(
        "      请求/秒: {:.1}",
        data_point.performance_metrics.requests_per_second
    );
    println!(
        "      错误率: {:.2}%",
        data_point.performance_metrics.error_rate * 100.0
    );
    println!(
        "      吞吐量: {:.1}",
        data_point.performance_metrics.throughput
    );

    // 异常检测
    let anomaly_emoji = if data_point.anomaly_results.anomalies_detected {
        "🚨"
    } else {
        "✅"
    };
    ...
    println!(
        "      异常数量: {}",
        data_point.anomaly_results.anomaly_count
    );
    println!(
        "      检测器数量: {}",
        data_point.anomaly_results.total_detectors
    );
    println!(
        "      异常评分: {:.3}",
        data_point.anomaly_results.anomaly_score
    );
...
fn display_dashboard_status(status: &DashboardStatus) {
    ...
    println!(
        "      最后更新: {}",
        chrono::DateTime::<chrono::Utc>::from(status.last_updated).format("%H:%M:%S")
    );
}
...
fn display_statistics_summary(summary: &DashboardStatisticsSummary) {
    ...
    println!(
        "      平均错误率: {:.2}%",
        summary.average_error_rate * 100.0
    );
    println!(
        "      异常检测率: {:.2}%",
        summary.anomaly_detection_rate * 100.0
    );
...
fn analyze_history_data(history_data: &[MonitoringDataPoint]) {
    ...
    // 分析资源使用趋势
    let cpu_values: Vec<f64> = history_data
        .iter()
        .map(|p| p.resource_usage.cpu_usage_percent)
        .collect();
    let memory_values: Vec<f64> = history_data
        .iter()
    ...
    // 分析异常检测结果
    let anomaly_count = history_data
        .iter()
