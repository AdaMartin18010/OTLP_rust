...
async fn strategy_pattern_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, protocol) in strategies {
        ...
        let result = client
            .send_trace(format!("{}-operation", name))
            .await?
...
async fn observer_pattern_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        let result = client
            .send_trace(format!("observed-operation-{}", i))
            .await?
        ...
        // 通知观察者
        metrics_observer
            .notify_operation_completed(result.success_count as usize)
            .await;
        ...
        println!(
            "  👁️ 观察指标 - 总操作数: {}, 成功率: {:.2}%",
            current_metrics.total_operations, current_metrics.success_rate
        );
...
async fn pool_pattern_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10 {
        ...
        let task = tokio::spawn(async move {
            ...
            let result = client
                .send_trace(format!("pool-operation-{}", i))
                .await?
    ...
    for i in 0..3 {
        match circuit_breaker
            .execute(|| {
                ...
                async move {
                    client
                        .send_trace(format!("normal-operation-{}", i))
                        .await?
        ...
        match circuit_breaker
            .execute(|| {
                async move {
                    ...
                    invalid_client
                        .send_trace(format!("failing-operation-{}", i))
                        .await?
                        .finish()
                        .await
                }
...
impl TelemetryDataBuilder {
    ...
    fn with_span_kind(mut self, kind: SpanKind) -> Self {
        if let otlp::data::TelemetryContent::Trace(ref mut trace_data) = self.data.content {
    ...
    fn with_event(
        mut self,
        name: &str,
        attributes: HashMap<String, otlp::data::AttributeValue>,
    ) -> Self {
...
impl ClientFactory {
    async fn create_client(
        ...
        let config = match client_type {
            ClientType::HighPerformance => OtlpConfig::default()
                .with_endpoint("http://localhost:4317")
                .with_protocol(TransportProtocol::Grpc)
            ...
            ClientType::Standard => OtlpConfig::default()
                .with_endpoint("http://localhost:4317")
                .with_protocol(TransportProtocol::Grpc)
                .with_service(service_name, service_version),
            ClientType::Lightweight => OtlpConfig::default()
                .with_endpoint("http://localhost:4317")
                .with_protocol(TransportProtocol::Http)
                .with_service(service_name, service_version)
                .with_batch_config(BatchConfig {
...
impl CircuitBreaker {
    ...
    where
        F: FnOnce() -> Fut,
        Fut: std::future::Future<Output = Result<T, otlp::error::OtlpError>>,
    {
