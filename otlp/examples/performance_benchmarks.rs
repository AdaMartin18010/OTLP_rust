...
use otlp::{
    microservices::{
        ...
        CircuitBreaker, CircuitBreakerConfig, Retryer, RetryConfig,
        ServiceEndpoint, HealthStatus, MicroserviceClient, MockConsulClient,
    },
    benchmarks::{
        BenchmarkRunner, BenchmarkConfig, BenchmarkResult,
        MicroserviceBenchmark, LoadBalancerBenchmark, TracingBenchmark,
    },
};
...
async fn run_microservice_benchmark() -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    ...
    info!("📊 微服务性能基准测试结果:");
    info!("  总迭代次数: {}", result.iterations_completed + result.iterations_failed);
    info!("  成功率: {:.2}%", 
...
async fn run_load_balancer_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let round_robin_result = round_robin_runner.run(|_iteration| async {
    ...
    let weighted_result = weighted_runner.run(|_iteration| async {
        let start = std::time::Instant::now();
...
async fn run_circuit_breaker_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = runner.run(|iteration| async move {
        ...
        // 测试熔断器调用
        let _result = circuit_breaker.call(|| {
...
async fn run_comprehensive_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = runner.run(|iteration| async move {
        ...
        mock_consul.add_service("test-service".to_string(), test_endpoints).await;
        ...
        // 模拟成功的服务调用
        let _response = client.call_service("test-service", move |_endpoint| {
            let iteration = iteration;
            Box::pin(async move {
                // 模拟服务调用
...
/// 导出基准测试报告
async fn export_benchmark_report(results: &[BenchmarkResult]) -> Result<(), Box<dyn std::error::Error>> {
    info!("📄 导出基准测试报告");
