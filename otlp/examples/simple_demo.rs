use otlp::{
    TelemetryData,
    TraceData, MetricData, LogData,
    AttributeValue, StatusCode
};
use otlp::data::{
    TelemetryContent, SpanKind, MetricType, LogSeverity, 
    SpanStatus, DataPoint, DataPointValue
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    trace_attributes.insert("user.id".to_string(), AttributeValue::String("12345".to_string()));
    trace_attributes.insert("request.method".to_string(), AttributeValue::String("GET".to_string()));
    trace_attributes.insert("request.path".to_string(), AttributeValue::String("/api/demo".to_string()));
    ...
    let trace_data = TraceData {
        ...
        start_time: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_nanos() as u64,
        end_time: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_nanos() as u64 + 150_000_000, // 150ms
        status: SpanStatus {
            code: StatusCode::Ok,
            message: None,
        },
    ...
    metric_attributes.insert("environment".to_string(), AttributeValue::String("demo".to_string()));
    metric_attributes.insert("version".to_string(), AttributeValue::String("1.0.0".to_string()));
    ...
    log_attributes.insert("logger.name".to_string(), AttributeValue::String("demo_logger".to_string()));
    log_attributes.insert("thread.id".to_string(), AttributeValue::String("main".to_string()));
    log_attributes.insert("application".to_string(), AttributeValue::String("otlp_demo".to_string()));
    ...
    for i in 0..3 {
        ...
        let batch_trace = TraceData {
            ...
            start_time: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_nanos() as u64,
            end_time: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_nanos() as u64 + 100_000_000, // 100ms
            status: SpanStatus {
                code: StatusCode::Ok,
                message: None,
            },
