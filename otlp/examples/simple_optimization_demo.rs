...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 3. 演示内存池优化
    demonstrate_memory_pool_optimization()?;
    println!("✅ 内存池优化演示完成");
...
/// 演示内存池优化功能（暂时禁用）
fn demonstrate_memory_pool_optimization() -> Result<(), Box<dyn std::error::Error>> {
    println!("\n🧠 内存池优化演示（暂时禁用）");
    println!("------------------");

    println!("  内存池优化功能暂时禁用，以避免线程安全问题");
    println!("  在实际应用中，可以使用标准的内存分配器或第三方内存池库");
...
fn run_performance_benchmarks() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 内存池性能测试（暂时禁用）
    // let mut memory_manager = AdvancedMemoryStrategyManager::new();
    // let start_time = Instant::now();
    // for _ in 0..10000 {
    //     let ptr = memory_manager.smart_allocate(1024)?;
    //     memory_manager.smart_deallocate(1024, ptr);
    // }
    // let memory_time = start_time.elapsed();
    // println!("  内存池分配/释放 10,000 次: {:?}", memory_time);
    println!("  内存池性能测试暂时禁用");
