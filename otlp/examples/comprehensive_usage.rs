...
async fn basic_usage_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let trace_result = client
        .send_trace("user-authentication")
        .await?
    ...
    let log_result = client
        .send_log("用户认证成功", LogSeverity::Info)
        .await?
...
async fn concurrent_processing_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10 {
        ...
        let result = client
            .send_metric("stream_metric", i as f64)
            .await?
...
async fn monitoring_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10 {
        let result = client
            .send_trace(format!("monitoring-operation-{}", i))
            .await?
    ...
    println!(
        "    - 处理的数据量: {}",
        metrics.processor_metrics.total_processed
    );
    println!(
        "    - 批处理次数: {}",
        metrics.processor_metrics.batch_count
    );
    println!(
        "    - 平均批处理大小: {:.2}",
        metrics.processor_metrics.average_batch_size
    );
    println!(
        "    - 过滤的数据量: {}",
        metrics.processor_metrics.total_filtered
    );
