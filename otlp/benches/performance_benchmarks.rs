...
fn benchmark_serialization_performance(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("json_deserialization", size), size, |b, _| {
            b.iter(|| {
                let deserialized: Vec<TelemetryData> = serde_json::from_slice(black_box(&serialized_data)).unwrap();
    ...
    group.bench_function("concurrent_validation", |b| {
        b
            .iter(|| async {
                ...
                let handles: Vec<_> = test_data.chunks(10)
                    .map(|chunk| {
