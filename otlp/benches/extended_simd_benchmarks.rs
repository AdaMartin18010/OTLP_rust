...
fn bench_performance_comparison(c: &mut Criterion) {
    ...
    for operation in operations.iter() {
        group.bench_function(
            format!("operation_{:?}", operation),
            |bencher| {
                bencher.iter(|| {
                    unsafe {
                        optimizer.process_f64_array_simd(black_box(&data), *operation)
                    }
                })
            },
        );
