...
impl Clone for MemoryPoolStats {
    fn clone(&self) -> Self {
        Self {
            ...
            total_bytes_allocated: AtomicUsize::new(self.total_bytes_allocated.load(Ordering::Relaxed)),
...
impl MemoryPool {
    ...
    fn maybe_cleanup(&self) {
        ...
        if removed > 0 {
            self.stats.current_pool_size.fetch_sub(removed, Ordering::Relaxed);
        }
    ...
    pub fn resize(&self, new_size: usize) {
        ...
        if new_size > blocks.len() {
            ...
            self.stats.current_pool_size.fetch_add(to_add, Ordering::Relaxed);
            ...
            self.stats.current_pool_size.fetch_sub(to_remove, Ordering::Relaxed);
        }
    }
    ...
    pub fn clear(&self) {
        ...
        let size = blocks.len();
        blocks.clear();
        self.stats.current_pool_size.fetch_sub(size, Ordering::Relaxed);
    }
}
...
mod tests {
    ...
    fn test_memory_allocation() {
        ...
        let stats = pool.stats();
        assert!(stats.pool_hits.load(Ordering::Relaxed) > 0 || stats.pool_misses.load(Ordering::Relaxed) > 0);
    }
