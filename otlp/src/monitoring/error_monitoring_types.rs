...
impl ErrorMetricsCollector {
    pub fn new(config: ErrorMetricsConfig) -> Result<Self> {
        Ok(Self {
            config,
...
pub struct ErrorMonitoringConfig {
    ...
    pub metrics: ErrorMetricsConfig,
...
pub enum DashboardUpdate {
    ...
    MetricsUpdated(ErrorMonitoringMetrics),
}
...
impl ErrorMonitoringSystem {
    /// 创建新的错误监控系统
    pub fn new(config: ErrorMonitoringConfig) -> Result<Self> {
        ...
        let metrics_collector = Arc::new(ErrorMetricsCollector::new(config.metrics.clone())?);
    ...
    /// 获取监控指标
    pub async fn get_metrics(&self) -> Result<ErrorMonitoringMetrics> {
        let metrics = ErrorMonitoringMetrics {
