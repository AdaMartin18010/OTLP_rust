...
async fn test_simple_component_basic_functionality() {
    let component =
        SimpleOtlpComponent::new("test_component".to_string(), Duration::from_millis(1));
...
async fn test_simple_component_error_handling() {
    let component = SimpleOtlpComponent::new("error_test".to_string(), Duration::from_millis(1));
...
async fn test_simple_component_performance() {
    ...
    let component =
        SimpleOtlpComponent::new("performance_test".to_string(), Duration::from_millis(1));
    ...
    for _ in 0..10 {
        tester
            .measure_async(|| component.process(test_data))
            .await
            .expect("Should process data");
    }
    ...
    assert!(
        ...
        "Average duration should be less than 10ms, got {:?}",
        avg_duration
    );
    assert!(
        max_duration < Duration::from_millis(20),
        "Max duration should be less than 20ms, got {:?}",
        max_duration
    );
    assert!(
        min_duration >= Duration::from_millis(1),
        "Min duration should be at least 1ms, got {:?}",
        min_duration
    );
...
async fn test_simple_component_concurrent_processing() {
    ...
    for i in 0..concurrent_tasks {
        ...
        let handle = tokio::spawn(async move { component.process(&data).await });
        handles.push(handle);
    }
...
async fn test_simple_component_throughput() {
    ...
    let component =
        SimpleOtlpComponent::new("throughput_test".to_string(), Duration::from_millis(1));
    ...
    assert!(
        throughput > 10.0,
        "Throughput should be greater than 10 ops/sec, got {:.2}",
        throughput
    );
...
async fn test_simple_component_memory_usage() {
    let component = SimpleOtlpComponent::new("memory_test".to_string(), Duration::from_millis(1));
...
async fn test_simple_component_stress() {
    ...
    assert!(
        operations > 10,
        "Should complete more than 10 operations in stress test, got {}",
        operations
    );
    assert!(
        throughput > 5.0,
        "Stress test throughput should be greater than 5 ops/sec, got {:.2}",
        throughput
    );
...
async fn test_simple_component_configuration() {
    // 测试配置
    let component = SimpleOtlpComponent::new("config_test".to_string(), Duration::from_millis(5));
    ...
    assert!(
        ...
        "Processing time should be at least 5ms as configured, got {:?}",
        duration
    );
}
...
async fn test_simple_component_integration() {
    // 集成测试：多个组件协作
    let processor = SimpleOtlpComponent::new("processor".to_string(), Duration::from_millis(1));

    let validator = SimpleOtlpComponent::new("validator".to_string(), Duration::from_millis(1));
    ...
    println!(
        "  Validated: {:?}",
        String::from_utf8_lossy(&validated_data)
    );
    println!(
        "  Processed: {:?}",
