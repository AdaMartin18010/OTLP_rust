...
fn test_batch_processing_performance() {
    ...
    // 创建测试数据
    let items: Vec<OtlpDataItem> = (0..50).map(|i| {
        let mut attributes = HashMap::new();
...
fn test_memory_pool_optimization() {
    ...
    // 处理大量数据以测试内存池
    let items: Vec<OtlpDataItem> = (0..1000).map(|i| OtlpDataItem {
        timestamp: i as u64,
        value: i as f64,
...
fn test_cache_optimization() {
    ...
    let items: Vec<OtlpDataItem> = (0..100).map(|i| OtlpDataItem {
        timestamp: i as u64,
        value: i as f64,
...
fn test_performance_optimization() {
    ...
    // 处理数据以创建内存压力
    let items: Vec<OtlpDataItem> = (0..5000).map(|i| OtlpDataItem {
        timestamp: i as u64,
        value: i as f64,
        attributes: HashMap::new(),
        resource_attributes: HashMap::new(),
...
fn test_large_batch_processing() {
    ...
    // 创建大批量数据
    let items: Vec<OtlpDataItem> = (0..5000).map(|i| {
        let mut attributes = HashMap::new();
