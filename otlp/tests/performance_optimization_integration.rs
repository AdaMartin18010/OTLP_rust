...
async fn test_simd_optimization_integration() {
    ...
    unsafe {
        let result = optimizer.process_i32_array_simd(&int_data, SimdIntOperation::Add(5)).unwrap();
...
async fn test_comprehensive_optimization_integration() {
    ...
    unsafe {
        let result = optimizer.simd_optimizer().process_f64_array_simd(&data, SimdOperation::Square).unwrap();
        assert_eq!(result.len(), 1000);
    }
    
    // 测试缓存管理器
    let test_data = vec![0u8; 1024];
    let metrics = optimizer.cache_manager().analyze_cache_performance(&test_data);
    assert!(metrics.data_size > 0);
...
fn test_performance_improvement() {
    ...
    unsafe {
        let _result = optimizer.process_f64_array_simd(&data, SimdOperation::Square).unwrap();
    }
...
fn test_memory_alignment() {
    ...
    for size in [64, 256, 1024, 4096, 16384] {
        ...
        // 验证内存对齐
        assert_eq!(ptr_addr % 64, 0, "内存未按64字节对齐: size={}, addr={}", size, ptr_addr);
...
fn test_simd_fallback() {
    ...
    let result_scalar = unsafe { optimizer.process_f64_array_simd(&data, SimdOperation::Square).unwrap() };
