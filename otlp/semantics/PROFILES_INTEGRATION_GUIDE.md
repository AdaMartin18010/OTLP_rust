...
- [Profiles集成完整指南](#profiles集成完整指南)
  - [📋 目录](#-目录)
  - [第一部分：Profiles概述](#第一部分profiles概述)
    - [1.1 什么是Profiles?](#11-什么是profiles)
      - [四支柱对比](#四支柱对比)
      - [Profiles的价值](#profiles的价值)
    - [1.2 Profiles类型](#12-profiles类型)
    - [1.3 Profiles在OTLP中的位置](#13-profiles在otlp中的位置)
  - [第二部分：数据模型](#第二部分数据模型)
    - [2.1 OTLP Profiles数据结构](#21-otlp-profiles数据结构)
    - [2.2 pprof数据格式](#22-pprof数据格式)
    - [2.3 Rust数据模型](#23-rust数据模型)
  - [第三部分：采集方案](#第三部分采集方案)
    - [3.1 Go应用采集](#31-go应用采集)
      - [3.1.1 使用pprof](#311-使用pprof)
      - [3.1.2 持续性能剖析配置](#312-持续性能剖析配置)
    - [3.2 Java应用采集](#32-java应用采集)
    - [3.3 Python应用采集](#33-python应用采集)
    - [3.4 Rust应用采集](#34-rust应用采集)
  - [第四部分：传输与存储](#第四部分传输与存储)
    - [4.1 Collector配置](#41-collector配置)
    - [4.2 存储后端选择](#42-存储后端选择)
      - [4.2.1 Pyroscope](#421-pyroscope)
      - [4.2.2 Grafana Phlare](#422-grafana-phlare)
  - [第五部分：查询与分析](#第五部分查询与分析)
    - [5.1 查询API](#51-查询api)
    - [5.2 火焰图可视化](#52-火焰图可视化)
  - [第六部分：与Traces关联](#第六部分与traces关联)
    - [6.1 自动关联](#61-自动关联)
    - [6.2 手动关联](#62-手动关联)
    - [6.3 查询关联的Profiles](#63-查询关联的profiles)
  - [第七部分：性能优化](#第七部分性能优化)
    - [7.1 采样策略](#71-采样策略)
    - [7.2 资源控制](#72-资源控制)
    - [7.3 性能指标](#73-性能指标)
