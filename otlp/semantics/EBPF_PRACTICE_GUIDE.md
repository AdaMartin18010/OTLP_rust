...
- [eBPF实践完整指南](#ebpf实践完整指南)
  - [📋 目录](#-目录)
  - [第一部分：eBPF概述](#第一部分ebpf概述)
    - [1.1 什么是eBPF?](#11-什么是ebpf)
      - [核心特性](#核心特性)
      - [eBPF在可观测性中的优势](#ebpf在可观测性中的优势)
    - [1.2 eBPF架构](#12-ebpf架构)
    - [1.3 eBPF程序类型](#13-ebpf程序类型)
  - [第二部分：eBPF与可观测性](#第二部分ebpf与可观测性)
    - [2.1 eBPF采集架构](#21-ebpf采集架构)
    - [2.2 零代码可观测性](#22-零代码可观测性)
  - [第三部分：eBPF Traces采集](#第三部分ebpf-traces采集)
    - [3.1 HTTP追踪](#31-http追踪)
      - [3.1.1 eBPF程序(HTTP追踪)](#311-ebpf程序http追踪)
      - [3.1.2 用户空间处理(Rust)](#312-用户空间处理rust)
    - [3.2 gRPC追踪](#32-grpc追踪)
    - [3.3 Database追踪](#33-database追踪)
  - [第四部分：eBPF Metrics采集](#第四部分ebpf-metrics采集)
    - [4.1 TCP连接指标](#41-tcp连接指标)
    - [4.2 文件I/O指标](#42-文件io指标)
  - [第五部分：eBPF Logs采集](#第五部分ebpf-logs采集)
    - [5.1 系统调用日志](#51-系统调用日志)
    - [5.2 安全审计日志](#52-安全审计日志)
  - [第六部分：eBPF Profiles采集](#第六部分ebpf-profiles采集)
    - [6.1 CPU性能剖析](#61-cpu性能剖析)
    - [6.2 Off-CPU分析](#62-off-cpu分析)
  - [第七部分：生产环境部署](#第七部分生产环境部署)
    - [7.1 OTel Collector集成](#71-otel-collector集成)
