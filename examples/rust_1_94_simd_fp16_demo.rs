...
impl CpuCapabilities {
    pub fn describe(&self) -> String {
        ...
        if self.has_fp16 { features.push("FP16"); }
        if self.has_avx512 { features.push("AVX-512"); }
        if self.has_avx2 { features.push("AVX2"); }
        if self.has_neon { features.push("NEON"); }
...
pub fn aggregate_fp16(samples: &[MetricSample]) -> AggregationResult {
    ...
    let fp16_values: Vec<Fp16> = samples.iter()
...
impl MetricBatchProcessor {
    ...
    pub fn process_batch(&self, samples: &[MetricSample]) -> Vec<AggregationResult> {
        ...
        println!("   Processed {} samples in {:?}", samples.len(), total_duration);
    ...
    pub fn compute_histogram(&self, samples: &[MetricSample], buckets: &[f64]) -> Vec<usize> {
        ...
        if self.use_fp16 {
            ...
            let bucket_thresholds: Vec<Fp16> = buckets.iter()
                .map(|&b| Fp16::from_f32(b as f32))
                .collect();
...
pub fn run_benchmarks(sample_counts: &[usize]) -> Vec<BenchmarkResult> {
    ...
    println!("\n   Running benchmarks with sample counts: {:?}", sample_counts);
    println!("   {}", "-".repeat(80));
...
fn benchmark_aggregation(
    ...
    let throughput_mops = (samples.len() * iterations) as f64 / total_duration.as_secs_f64() / 1_000_000.0;
...
fn print_benchmark_result(result: &BenchmarkResult, sample_count: usize) {
    println!("   {:<20} samples={:<8} time={:>10.2}μs  throughput={:>6.2} Mops/s  bw={:>4.1} GB/s",
        result.name,
        sample_count,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   FP16 support: {}", if capabilities.has_fp16 { "✅ Yes" } else { "❌ No" });
    ...
    println!("   {:<15} {:<15} {:<15}", "Original (f32)", "FP16 Bits", "Recovered (f32)");
    println!("   {}", "-".repeat(50));
    
    for &value in &test_values {
        let fp16 = Fp16::from_f32(value);
        let recovered = fp16.to_f32();
        println!("   {:<15} {:<15} {:<15.6}", value, format!("0x{:04X}", fp16.to_bits()), recovered);
    }
    ...
    println!("   Scalar FP64:");
    println!("      Sum: {:.2}, Mean: {:.2}, Min: {:.2}, Max: {:.2}",
    ...
    for (i, &count) in counts.iter().enumerate() {
        if i < buckets.len() {
            ...
            println!("      > {:6.1}: {:5} samples (overflow)", buckets.last().unwrap(), count);
        }
    }
    ...
    println!("   FP64 (f64):  {:>8} bytes ({:>5.1} MB)", fp64_bytes, fp64_bytes as f64 / 1_048_576.0);
    println!("   FP32 (f32):  {:>8} bytes ({:>5.1} MB)", fp32_bytes, fp32_bytes as f64 / 1_048_576.0);
    println!("   FP16:        {:>8} bytes ({:>5.1} MB)", fp16_bytes, fp16_bytes as f64 / 1_048_576.0);
    println!("   FP16 savings vs FP64: {:.1}%", (1.0 - fp16_bytes as f64 / fp64_bytes as f64) * 100.0);
    println!("   FP16 savings vs FP32: {:.1}%", (1.0 - fp16_bytes as f64 / fp32_bytes as f64) * 100.0);
...
mod tests {
    ...
    fn test_fp16_conversions() {
        ...
        for &original in &test_values {
            ...
            assert!(relative_error < 0.01 || original == 0.0, 
                "Failed for {}: got {}, relative error {}", original, recovered, relative_error);
        }
    }
    ...
    fn test_feature_detection() {
        ...
        assert!(caps.has_sse2 || caps.has_neon || caps.has_avx2 || caps.has_avx512 || !caps.has_fp16);
